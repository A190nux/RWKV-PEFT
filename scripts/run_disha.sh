load_model='/kaggle/working/rwkv-3b-stage2.pth'
proj_dir='/kaggle/working/'
data_file='/kaggle/input/cn-en-novels-dataset/data_binidx_text_document'

n_layer=32
n_embd=2560

micro_bsz=1
epoch_save=1
epoch_steps=50
ctx_len=6400

disha_config='{"mode":"bat","load":"","r":64}'


python train.py --load_model $load_model \
--proj_dir $proj_dir --data_file $data_file \
--vocab_size 65536 \
--n_layer $n_layer --n_embd $n_embd \
--data_type binidx --dataload pad --loss_mask pad \
--ctx_len $ctx_len --micro_bsz $micro_bsz \
--epoch_steps $epoch_steps --epoch_count 35 --epoch_begin 0 --epoch_save $epoch_save \
--lr_init 2e-5 --lr_final 2e-7 --warmup_steps 50 --beta1 0.9 --beta2 0.99 --adam_eps 1e-8 \
--accelerator gpu --devices 1 --precision bf16 --strategy deepspeed_stage_1 --grad_cp 1 \
--my_testing "x070" \
--peft disha --disha_config $disha_config
